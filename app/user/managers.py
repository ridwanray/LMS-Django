...
class CustomUserManager(BaseUserManager):
    ...
    def create_app_user(self, email, roles=None,**extra_fields):
            from .utils import create_token_and_send_user_email
            roles = roles if roles else ['STUDENT']
            if not email:
                raise ValueError(_("The Email must be set"))
            email = self.normalize_email(email)
            user = self.model(email=email, roles=roles,  **extra_fields)
            user.save()
            create_token_and_send_user_email(user, token_type=TokenTypeClass.ACCOUNT_VERIFICATION)
            return user
